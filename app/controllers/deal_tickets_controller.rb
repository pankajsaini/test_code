...
class DealTicketsController < ApplicationController
  def show
    ...
    people_associated_deal = get_deal_details(pipeline_obj,api_key,deal_id)
    deal_response(people_associated_deal,pipeline_obj)
  end


  def deal_response(people_associated_deal,pipeline_obj)
...
def get_deal_tickets(associated_people)
  ...
  def get_deal_details(pipeline_obj,api_key,deal_id)
